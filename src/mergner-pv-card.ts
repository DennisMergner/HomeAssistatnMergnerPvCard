...
class MergnerPvCardEditor extends HTMLElement {
      ...
      <div class="entity-picker" data-picker-id="${this.safeText(selectorId)}" data-field="${String(field)}">
        ...
        <div class="picker-dropdown" role="listbox">
          ...
          <div class="picker-options">
            ...
            ${renderGroup(rest, "Alle Entitäten")}
          </div>
        </div>
        ` : ""}
      </div>
  ...
  private wireEvents(nodes: FlowNode[], links: FlowLink[]): void {
    ...
    root.querySelectorAll<HTMLElement>(".entity-picker").forEach((picker) => {
      ...
      picker.querySelector<HTMLButtonElement>(".picker-trigger")?.addEventListener("click", () => {
    ...
    root.querySelectorAll<HTMLInputElement>("input[data-action='entity-search']").forEach((searchInput) => {
      searchInput.addEventListener("input", () => {
        ...
        const select = root.querySelector<HTMLSelectElement>(`select[data-entity-select-id='${target}']`);
        if (!select) {
          return;
        }
