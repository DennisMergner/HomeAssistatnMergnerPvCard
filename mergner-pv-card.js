      ...
      <article class="node node-${a} ${c}" style="--node-size:${s}%; left:${this.clampPercent(e.x)}%; top:${this.clampPercent(e.y)}%;">
        <div class="node-orb ${l?"has-image":""}">
          ...
          <div class="node-overlay">
            ...
            ${t?`<div class="node-value node-chip">${this.safeText(this.formatMetricValue(t.value,t.unit))}</div>`:""}
            ${t?`<div class="node-value-label node-chip">${this.safeText(t.label)}</div>`:""}
            ${g}
          </div>
        </div>
