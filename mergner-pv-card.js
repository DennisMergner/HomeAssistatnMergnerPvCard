var v=[{id:"solar",name:"Solar",role:"pv",entityLabel:"Leistung",secondaryLabel:"Heute",size:120,x:20,y:20},{id:"battery",name:"Batterie",role:"battery",entityLabel:"Laden / Entladen",secondaryLabel:"SOC",secondaryUnit:"%",tertiaryLabel:"Heute",size:120,x:80,y:20},{id:"house",name:"Haus",role:"house",entityLabel:"Verbrauch",secondaryLabel:"Heute",size:120,x:20,y:80},{id:"grid",name:"Netz",role:"grid",entityLabel:"Bezug / Einspeisung",secondaryLabel:"Heute",size:120,x:80,y:80}],x=[{from:"solar",to:"house",entity:"sensor.pv_to_house_power"},{from:"solar",to:"battery",entity:"sensor.pv_to_battery_power"},{from:"battery",to:"house",entity:"sensor.battery_to_house_power"},{from:"grid",to:"house",entity:"sensor.grid_to_house_power"}],y=class E extends HTMLElement{_config;_hass;static getConfigElement(){return document.createElement("mergner-pv-card-editor")}static getStubConfig(){return{type:"custom:mergner-pv-card",title:"PV Flow",nodes:v,links:x}}setConfig(e){if(!e||e.type!=="custom:mergner-pv-card")throw new Error("Card type must be custom:mergner-pv-card");this._config=e,this.render()}set hass(e){this._hass=e,this.render()}getCardSize(){return 5}connectedCallback(){this.render()}safeText(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}clampPercent(e){return Number.isNaN(e)?50:Math.max(2,Math.min(98,e))}clampMeterPercent(e){return Number.isNaN(e)?0:Math.max(0,Math.min(100,e))}clampNodeSize(e){return Number.isNaN(e)?120:Math.max(40,Math.min(320,e))}getEntity(e){if(!(!e||!this._hass?.states?.[e]))return this._hass.states[e]}getState(e){return this.getEntity(e)?.state??"n/a"}getUnit(e){let a=this.getEntity(e)?.attributes?.unit_of_measurement;return typeof a=="string"?a:""}parseNumber(e){let i=this.getState(e),a=Number.parseFloat(i);return Number.isFinite(a)?a:0}getNodeRole(e){return e.role??"custom"}roleLabel(e){switch(e){case"pv":return"PV";case"battery":return"Batterie";case"house":return"Haus";case"grid":return"Netz";case"inverter":return"Wechselrichter";default:return"Knoten"}}defaultMetricLabel(e,i){if(i==="primary")switch(e){case"pv":return"Leistung";case"battery":return"Laden / Entladen";case"house":return"Verbrauch";case"grid":return"Bezug / Einspeisung";case"inverter":return"Leistung";default:return"Wert"}if(i==="secondary")switch(e){case"battery":return"SOC";case"pv":case"house":case"grid":case"inverter":return"Heute";default:return"Detail"}return e==="battery"?"Heute":"Extra"}formatMetricValue(e,i){let a=e.trim(),t=i.trim();return t?`${a} ${t}`:a}getNodeMetrics(e){let i=this.getNodeRole(e);return[{entity:e.entity,label:e.entityLabel,unit:e.unit,defaultLabel:this.defaultMetricLabel(i,"primary"),showWhenEmpty:!0},{entity:e.secondaryEntity,label:e.secondaryLabel,unit:e.secondaryUnit,defaultLabel:this.defaultMetricLabel(i,"secondary"),showWhenEmpty:!1},{entity:e.tertiaryEntity,label:e.tertiaryLabel,unit:e.tertiaryUnit,defaultLabel:this.defaultMetricLabel(i,"tertiary"),showWhenEmpty:!1}].filter(t=>t.showWhenEmpty||!!t.entity?.trim()).map(t=>{let r=t.entity?this.getState(t.entity):"n/a",n=t.unit??(t.entity?this.getUnit(t.entity):"");return{label:t.label?.trim()||t.defaultLabel,value:r,numericValue:t.entity?this.parseNumber(t.entity):Number.NaN,unit:n}})}getBatteryLevel(e){let i=e.find(a=>a.unit==="%"||/soc|state of charge|akku|charge|level/i.test(a.label));if(!(!i||Number.isNaN(i.numericValue)))return this.clampMeterPercent(i.numericValue)}getSummaryUnit(e){for(let i of e){let a=i.unit?.trim()||this.getUnit(i.entity);if(a)return a}return""}renderSummary(e){let a=[{role:"pv",label:"Erzeugung",className:"pv"},{role:"house",label:"Verbrauch",className:"house"},{role:"battery",label:"Batterie",className:"battery"},{role:"grid",label:"Netz",className:"grid"}].map(t=>{let r=e.filter(s=>this.getNodeRole(s)===t.role&&s.entity?.trim());if(r.length===0)return"";let n=r.reduce((s,d)=>s+this.parseNumber(d.entity),0),o=this.getSummaryUnit(r),l=this.formatMetricValue(n.toFixed(Math.abs(n)>=100?0:1),o);return`
        ...
        `;return`
      <article class="node node-${i} ${p}" style="--node-size:${l}px; left:${this.clampPercent(e.x)}%; top:${this.clampPercent(e.y)}%;">
    ...
    `}getLineAnnotationOffset(e){return e==="bottom"?3.6:-3.6}getLinkValue(e){if(!e.valueEntity?.trim())return"";let i=this.getState(e.valueEntity),a=e.valueUnit??this.getUnit(e.valueEntity);return this.formatMetricValue(i,a)}resolveLinkDirection(e){if(!e.entity)return"idle";let i=this.parseNumber(e.entity);return e.invert&&(i=-i),i>0?"forward":i<0?"reverse":"idle"}renderLinks(e,i){let a=new Map(e.map(r=>[r.id,r]));return`<svg class="line-layer" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">${i.map(r=>{let n=a.get(r.from),o=a.get(r.to);if(!n||!o)return"";let l=(n.x+o.x)/2,s=(n.y+o.y)/2,d=o.x-n.x,p=o.y-n.y,u=Math.hypot(d,p)||1,g=-p/u,c=d/u,m=r.labelPosition??"top",f=r.valuePosition??"bottom",h=r.label?.trim()??"",b=this.getLinkValue(r),N=h&&b&&m===f?1.8:0,k=h&&b&&m===f?-1.8:0,L=this.getLineAnnotationOffset(m)+N,$=this.getLineAnnotationOffset(f)+k,C=l+g*L,M=s+c*L,T=l+g*$,S=s+c*$,z=this.resolveLinkDirection(r),_=h?`<title>${this.safeText(h)}</title>`:"",F=h?`<text class="flow-annotation flow-annotation-label" x="${C}" y="${M}" text-anchor="middle" dominant-baseline="middle">${this.safeText(h)}</text>`:"",P=b?`<text class="flow-annotation flow-annotation-value" x="${T}" y="${S}" text-anchor="middle" dominant-baseline="middle">${this.safeText(b)}</text>`:"";return`<g class="flow-edge"><line class="flow-line ${z}" x1="${n.x}" y1="${n.y}" x2="${o.x}" y2="${o.y}">${_}</line>${F}${P}</g>`}).join("")}</svg>`}render(){this.shadowRoot||this.attachShadow({mode:"open"});let e=this.shadowRoot;if(!e)return;let i=this.normalizeConfig(this._config??E.getStubConfig());e.innerHTML=`
      <style>
        :host {
          display: block;
        }
    ...
    `}},w=class extends HTMLElement{_config;_hass;_dragNodeIndex;_dragEventsBound=!1;_entityIdsSignature="";_layoutZoom=100;_layoutZoomMode="auto";clampEditorNodeSize(e){return Number.isNaN(e)?120:Math.max(40,Math.min(320,e))}safeText(e){return(typeof e=="string"?e:String(e??"")).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}clampEditorPercent(e){return Number.isNaN(e)?50:Math.max(2,Math.min(98,e))}normalizeEditorConfig(e){let i=y.getStubConfig(),a={...i,...e,title:(e.title??i.title??"PV Flow").toString()},r=(Array.isArray(e.nodes)&&e.nodes.length>0?e.nodes:i.nodes??[]).map((s,d)=>({...s,id:(s.id??`node_${d+1}`).toString().trim()||`node_${d+1}`,name:(s.name??`Node ${d+1}`).toString().trim()||`Node ${d+1}`,role:s.role??"custom",x:this.clampEditorPercent(Number(s.x)),y:this.clampEditorPercent(Number(s.y)),size:this.clampEditorNodeSize(Number(s.size??120))})),n=new Set(r.map(s=>s.id)),l=(Array.isArray(e.links)?e.links:i.links??[]).filter(s=>n.has(s.from)&&n.has(s.to));return{...a,nodes:r,links:l}}setConfig(e){this._config=this.normalizeEditorConfig(e),this.render()}set hass(e){let i=Object.keys(e?.states??{}).sort((t,r)=>t.localeCompare(r)).join("|"),a=!this._hass||i!==this._entityIdsSignature;this._hass=e,this._entityIdsSignature=i,a&&this.render()}connectedCallback(){this.bindDragEvents(),this.render()}get safeConfig(){return this._config??y.getStubConfig()}emitConfig(e){this._config=e,this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0})),this.render()}updateNode(e,i,a,t){let r=[...e];r[a]={...r[a],...t},this.emitConfig({...this.safeConfig,nodes:r,links:i})}bindDragEvents(){this._dragEventsBound||(window.addEventListener("pointermove",this.handlePointerMove),window.addEventListener("pointerup",this.handlePointerUp),window.addEventListener("pointercancel",this.handlePointerUp),this._dragEventsBound=!0)}disconnectedCallback(){this._dragEventsBound&&(window.removeEventListener("pointermove",this.handlePointerMove),window.removeEventListener("pointerup",this.handlePointerUp),window.removeEventListener("pointercancel",this.handlePointerUp),this._dragEventsBound=!1)}getEntityIds(){return Object.keys(this._hass?.states??{}).sort((e,i)=>e.localeCompare(i))}getEntityUnit(e){let a=this._hass?.states?.[e]?.attributes?.unit_of_measurement;return typeof a=="string"?a:""}getEntityDeviceClass(e){let a=this._hass?.states?.[e]?.attributes?.device_class;return typeof a=="string"?a:""}matchesEntityFilter(e,i){if(i==="any")return!0;let a=this.getEntityUnit(e).toLowerCase(),t=this.getEntityDeviceClass(e).toLowerCase();return i==="power"?/^(w|kw|mw|gw|va|kva)$/.test(a)||["power","apparent_power","reactive_power"].includes(t):i==="energy"?/^(wh|kwh|mwh|gwh)$/.test(a)||t==="energy":a==="%"||t==="battery"}getNodeEntityFilter(e,i){return i==="entity"?"power":i==="secondaryEntity"?e.role==="battery"?"percent":"energy":i==="tertiaryEntity"?"energy":"any"}renderEntitySelect(e,i,a,t="Select entity",r="any"){let n=this.getEntityIds(),o=a?.trim()??"",l=o&&!n.includes(o)?`<option value="${this.safeText(o)}" selected>${this.safeText(o)}</option>`:"",s=n.filter(c=>this.matchesEntityFilter(c,r)),d=n.filter(c=>!s.includes(c)),p=c=>c.map(m=>{let f=m===o?"selected":"";return`<option value="${this.safeText(m)}" ${f}>${this.safeText(m)}</option>`}).join(""),u=s.length>0?`<optgroup label="Recommended">${p(s)}</optgroup>`:"",g=d.length>0?`<optgroup label="All entities">${p(d)}</optgroup>`:"";return`
    ...
    `}renderLayoutCanvas(e,i){let a=new Map(e.map(o=>[o.id,o])),t=i.map(o=>{let l=a.get(o.from),s=a.get(o.to);return!l||!s?"":`<line x1="${l.x}" y1="${l.y}" x2="${s.x}" y2="${s.y}"></line>`}).join(""),r=this.getEffectiveLayoutZoom(e),n=e.map((o,l)=>{let s=o.image?.trim(),d=s?`<img src="${this.safeText(s)}" alt="${this.safeText(o.name)}" />`:`<span>${this.safeText(o.name.slice(0,1).toUpperCase())}</span>`,p=r/100,u=Math.max(24,Math.min(220,Math.round((o.size??120)*p)));return`
          <button
            class="layout-node"
    ...
    `}startNodeDrag(e){this._dragNodeIndex=e}getEffectiveLayoutZoom(e){if(this._layoutZoomMode==="manual")return this._layoutZoom;let i=Math.max(...e.map(n=>this.clampEditorNodeSize(Number(n.size??120))),120),a=e.length>=8?.78:e.length>=6?.86:e.length>=4?.93:1,r=Math.round(84/i*100*a);return Math.max(50,Math.min(160,r))}handlePointerMove=e=>{if(this._dragNodeIndex===void 0)return;let a=this.shadowRoot?.querySelector(".layout-canvas");if(!a)return;let t=a.getBoundingClientRect();if(t.width===0||t.height===0)return;let r=this.safeConfig.nodes&&this.safeConfig.nodes.length>0?this.safeConfig.nodes:v,n=this.safeConfig.links??x,o=Math.max(4,Math.min(96,(e.clientX-t.left)/t.width*100)),l=Math.max(4,Math.min(96,(e.clientY-t.top)/t.height*100));this.updateNode(r,n,this._dragNodeIndex,{x:Number(o.toFixed(1)),y:Number(l.toFixed(1))})};handlePointerUp=()=>{this._dragNodeIndex=void 0};readFileAsDataUrl(e){return new Promise((i,a)=>{let t=new FileReader;t.addEventListener("load",()=>{if(typeof t.result=="string"){i(t.result);return}a(new Error("Image upload failed"))}),t.addEventListener("error",()=>a(t.error??new Error("Image upload failed"))),t.readAsDataURL(e)})}renderNodeRows(e){let i=["pv","battery","house","grid","inverter","custom"];return e.map((a,t)=>`
            ...
            <div class="node-grid">
              ...
              <label>
                ...
                <select data-field="role">
                  ${i.map(r=>`<option value="${r}" ${(a.role??"custom")===r?"selected":""}>${r}</option>`).join("")}
                </select>
              </label>
              <label>
                <span>Image URL</span>
        ...
        `).join("")}renderLinkRows(e,i){let a=i.map(t=>`<option value="${this.safeText(t.id)}">${this.safeText(t.name)} (${this.safeText(t.id)})</option>`).join("");return e.map((t,r)=>`
          <div class="row" data-kind="link" data-index="${r}">
        ...
        `).join("")}wireEvents(e,i){let a=this.shadowRoot;a&&(a.querySelectorAll("input[data-action='entity-search']").forEach(t=>{t.addEventListener("input",()=>{let r=t.dataset.target;if(!r)return;let n=a.querySelector(`select[data-entity-select-id='${r}']`);if(!n)return;let o=t.value.trim().toLowerCase();n.querySelectorAll("option").forEach(l=>{if(!l.value){l.hidden=!1;return}let s=(l.textContent??"").toLowerCase(),d=l.value.toLowerCase(),p=l.selected;l.hidden=o.length>0&&!p&&!s.includes(o)&&!d.includes(o)})})}),a.querySelectorAll("input[data-action='layout-zoom']").forEach(t=>{t.addEventListener("input",()=>{let r=Number(t.value);Number.isFinite(r)&&(this._layoutZoomMode="manual",this._layoutZoom=Math.max(50,Math.min(160,r)),this.render())})}),a.querySelector("select[data-action='layout-zoom-mode']")?.addEventListener("change",t=>{let r=t.currentTarget;(r.value==="auto"||r.value==="manual")&&(this._layoutZoomMode=r.value,this.render())}),a.querySelectorAll(".node-card[data-kind='node']").forEach((t,r)=>{t.querySelectorAll("input[data-field], select[data-field]").forEach(n=>{n.addEventListener("change",()=>{let o=n.dataset.field,l=n instanceof HTMLInputElement&&n.type==="number"?Number(n.value):n.value;this.updateNode(e,i,r,{[o]:l})})}),t.querySelector("input[data-action='upload-image']")?.addEventListener("change",async n=>{let o=n.currentTarget,l=o.files?.[0];if(l)try{let s=await this.readFileAsDataUrl(l);this.updateNode(e,i,r,{image:s})}catch(s){console.error(s)}finally{o.value=""}}),t.querySelector("button[data-action='clear-image']")?.addEventListener("click",()=>{this.updateNode(e,i,r,{image:""})}),t.querySelector("button[data-action='remove-node']")?.addEventListener("click",()=>{let n=e.filter((s,d)=>d!==r),o=new Set(n.map(s=>s.id)),l=i.filter(s=>o.has(s.from)&&o.has(s.to));this.emitConfig({...this.safeConfig,nodes:n,links:l})})}),a.querySelectorAll("button[data-action='drag-node']").forEach(t=>{t.addEventListener("pointerdown",r=>{if(r.button!==0)return;let n=Number(t.dataset.index);Number.isFinite(n)&&(r.preventDefault(),this.startNodeDrag(n))})}),a.querySelectorAll(".row[data-kind='link']").forEach((t,r)=>{t.querySelectorAll("input[data-field], select[data-field]").forEach(n=>{if(n instanceof HTMLInputElement&&n.type==="checkbox"){n.addEventListener("change",()=>{let o=[...i];o[r]={...o[r],invert:n.checked},this.emitConfig({...this.safeConfig,nodes:e,links:o})});return}n.addEventListener("change",()=>{let o=n.dataset.field,l=[...i];l[r]={...l[r],[o]:n.value},this.emitConfig({...this.safeConfig,nodes:e,links:l})})}),t.querySelector("button[data-action='remove-link']")?.addEventListener("click",()=>{let n=i.filter((o,l)=>l!==r);this.emitConfig({...this.safeConfig,nodes:e,links:n})})}),a.querySelector("button[data-action='add-node']")?.addEventListener("click",()=>{let t=[...e,{id:`node_${e.length+1}`,name:`Node ${e.length+1}`,x:50,y:50}];this.emitConfig({...this.safeConfig,nodes:t,links:i})}),a.querySelector("button[data-action='add-link']")?.addEventListener("click",()=>{if(e.length<2)return;let t=[...i,{from:e[0].id,to:e[1].id,entity:"",invert:!1}];this.emitConfig({...this.safeConfig,nodes:e,links:t})}),a.querySelectorAll(".row[data-kind='link']").forEach((t,r)=>{let n=t.querySelectorAll("select[data-field]"),o=i[r];n[0]&&(n[0].value=o.from),n[1]&&(n[1].value=o.to)}))}render(){this.shadowRoot||this.attachShadow({mode:"open"});let e=this.shadowRoot;if(!e)return;let i=this.safeConfig.nodes&&this.safeConfig.nodes.length>0?this.safeConfig.nodes:v,a=this.safeConfig.links??x;e.innerHTML=`
      <style>
        :host {
          display: block;
          padding: 8px 0;
